  ...
  return (
    ...
    >
      {items.map((item: ISliderItem) => {
        return (
          <div key={item.name} className={clsx(styles.mobileDiv, "carousel-slide")}>
            <div
              className={clsx(
                "carousel-description absolute left-auto right-auto bottom-1/3 mb-10 text-left w-full lg:container px-4 md:px-10",
                styles.textBox
              )}
            >
              ...
              <Link
                to={`/${item.name}`}
                className={clsx("btn btn-sm lg:btn-md mt-3", { [styles.buttonDark]: !lightTheme })}
              >
                바로가기
                <svg
